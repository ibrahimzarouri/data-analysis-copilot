...
- **Natural language → code** — questions like *"Which region is most profitable?"* are translated into Pandas code automatically
...
|----------|----------|-------------|
| `API_KEY` | ✅ | API key for the LLM endpoint (OpenAI-compatible) |
| `BASE_URL` | ➖ | LLM endpoint URL (default: `https://chat-ai.academiccloud.de/v1`) |
| `MODEL` | ➖ | Model name (default: `qwen3-coder-30b-a3b-instruct`) |
| `E2B_API_KEY` | ➖ | [E2B](https://e2b.dev) key for sandboxed code execution. Without it, code runs locally (see [Security](#-security)) |
